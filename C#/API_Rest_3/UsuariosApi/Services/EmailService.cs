...
namespace UsuariosApi.Services
{
    public class EmailService
    {
        ...
        internal void EnviarEmail(string[] destinatario, string assunto, int id, string codigo)
        {
            Mensagem mensagem = new Mensagem(destinatario, assunto, id, codigo);
        ...
        private void EnviarEmail(MimeMessage mensagemDeEmail)
        {
            using (var client = new SmtpClient())
            {
                try
                {
                    // configurações de email em 'appsettings.json'
                    client.Connect(
